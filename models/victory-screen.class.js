...
class VictoryScreen {
  ...
  loadDanceImages() {
    for (let i = 1; i <= 12; i++) {
      ...
      image.src = `img/You won, you lost/victory_dance_${i}.png`;
  ...
  drawWinShadow() {
    ...
    this.ctx.fillText("YOU WON!", x + 8, y + 8);
  }
  ...
  drawWinText() {
    ...
    const gradient = this.ctx.createLinearGradient(0, y - 50, 0, y + 50);
  ...
  updateDance() {
    ...
    if (now - this.lastDanceFrame < this.danceFrameDelay) {
      return;
    }

    this.danceFrame++;

    if (this.danceFrame >= this.danceImages.length) {
      this.danceFrame = 0;
    }
  ...
  clearAnimationArea() {
    ...
    const y = this.canvas.height - height - 110;
    this.ctx.clearRect(x - 10, y - 10, width + 20, height + 20);
  }

  drawPepe() {
    const image = this.getCurrentDanceImage();
    if (!image) return;
    const position = this.getPepePosition();
  ...
  getCurrentDanceImage() {
    const image = this.danceImages[this.danceFrame];
  ...
  prepareConfettiPiece(piece) {
    ...
    this.ctx.fillStyle = this.getConfettiColor();
  }
